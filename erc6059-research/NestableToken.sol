...
contract NestableToken is Context, IERC165, IERC721, INestable {
    ...
    // Function to check address is contract address 
    ...
    function _nestTransfer(
        ...
        if (!isContract(to)) revert IsNotContract();
    ...
    function addChild(
        ...
        if (!isContract(childAddress)) revert IsNotContract();
    ...
    function transferChild(
        ...
        bool isPending,
        bytes memory data
    ) public payable  virtual onlyApprovedOrOwner(tokenId) {
        _transferChild(
            tokenId,
            to,
            destinationId,
